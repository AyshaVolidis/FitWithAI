...
export const getOverviewKPIs = createServerFn({ method: "GET" }).handler(async () => {
  ...
  return safeQuery(
    async () => {
      ...
      const calorieValues = ((caloriesRes.data ?? []) as any[])
        .map((p: any) => p.daily_calorie_target)
...
export const getWorkoutsPerDayThisWeek = createServerFn({ method: "GET" }).handler(async () => {
  ...
  return safeQuery(async () => {
    ...
    return Object.entries(counts).map(([date, count], i) => ({
      day: days[i] ?? date,
      count,
      date,
    }));
  ...
  .handler(async ({ data }) => {
    ...
    return safeQuery(async () => {
      const [workoutRes, foodRes, chatRes] = await Promise.all([
        client
          .from("workout_sessions")
          .select("id, user_id, status, created_at, date")
          .order("created_at", { ascending: false })
          .limit(data.limit),
        client
          .from("food_logs")
          .select("id, user_id, name, created_at, date")
          .order("created_at", { ascending: false })
          .limit(data.limit),
        client
          .from("chat_messages")
      ...
      for (const w of workoutRes.data ?? []) {
        ...
        activities.push({
          type: "workout",
          description: `${p?.name ?? "Someone"} ${w.status === "completed" ? "completed" : "started"} a workout`,
      ...
      for (const f of foodRes.data ?? []) {
        ...
        activities.push({
          type: "meal",
          description: `${p?.name ?? "Someone"} logged "${f.name}"`,
      ...
      for (const c of chatRes.data ?? []) {
        ...
        activities.push({
          type: "chat",
          description: `${p?.name ?? "Someone"} asked: "${snippet}"`,
    ...
    return safeQuery(
      async () => {
        ...
        if (data.filter === "active") {
          const today = new Date().toISOString().slice(0, 10);
          const { data: activeIdsRaw } = await client
            .from("daily_usage")
            .select("user_id")
            .eq("date", today);
          ...
          const inactiveIds = allProfilesArr
            .map((p: any) => p.id)
  ...
  .handler(async ({ data }) => {
    ...
    return safeQuery(async () => {
      ...
      const completedWorkouts =
        (allSessions ?? ([] as any[])).filter((s: any) => s.status === "completed").length ?? 0;
      const ratings = ((allSessions ?? []) as any[])
        .map((s: any) => s.rating)
        .filter(Boolean) as number[];
      const avgRating =
      ...
      return {
        ...
        stats: {
          ...
          completedWorkouts,
          avgRating,
          totalMessages: chatRes.data?.length ?? 0,
        },
      };
    }, emptyUserDetail());
  });
...
function emptyUserDetail() {
  return {
    ...
    recentChat: [],
    dailyUsage: [],
    stats: { totalWorkouts: 0, completedWorkouts: 0, avgRating: 0, totalMessages: 0 },
  };
}
...
export const getWorkoutStats = createServerFn({ method: "GET" }).handler(async () => {
  const client = admin();
  if (!client)
    return { totalSessions: 0, avgDuration: 0, mostCommonGoal: "none", completionRate: 0 };
    ...
    async () => {
      ...
      const avgDuration =
        durations && durations.length > 0
          ? Math.round(
              (durations as any[]).reduce((a: number, b: any) => a + (b.duration_actual ?? 0), 0) /
                durations.length,
            )
          : 0;
      ...
      const completionRate =
        totalSessions && totalSessions > 0 ? Math.round((totalCompleted / totalSessions) * 100) : 0;
      ...
      const mostCommonGoal =
        Object.entries(goalCounts).sort((a, b) => b[1] - a[1])[0]?.[0] ?? "none";
...
export const getTopExercises = createServerFn({ method: "GET" }).handler(async () => {
  ...
  return safeQuery(async () => {
    const { data: sessions } = await client.from("workout_sessions").select("plan_json").limit(500);

    const exerciseCounts: Record<string, { count: number; totalDuration: number }> = {};

    for (const s of sessions ?? []) {
      const plan = s.plan_json as unknown as {
        segments?: { title: string; duration_seconds: number }[];
      };
...
export const getMealLogs = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    return safeQuery(
      async () => {
        let query = client.from("food_logs").select("*", { count: "exact" });
...
export const getChatStats = createServerFn({ method: "GET" }).handler(async () => {
  const client = admin();
  if (!client)
    return { totalConversations: 0, avgMessagesPerSession: 0, avgDuration: 0, commonTopics: [] };
  ...
  return safeQuery(
    async () => {
      ...
      const avgMessagesPerSession =
        totalConversations > 0 ? Math.round((totalMessages ?? 0) / totalConversations) : 0;
      ...
      async () => {
        ...
        if (data.search) {
          ...
          conversations = conversations.filter(
            (c: any) =>
              c.userName.toLowerCase().includes(s) || c.userEmail.toLowerCase().includes(s),
          );
        }
...
export const getWorkoutTemplates = createServerFn({ method: "GET" }).handler(async () => {
  const client = admin();
  if (!client) return [];
  return safeQuery(async () => {
...
export const deleteWorkoutTemplate = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    return safeQuery(
      async () => {
        const { error } = await client.from("workout_templates").delete().eq("id", data.id);
        if (error) return { ok: false, error: error.message };
  ...
  return safeQuery(
    async () => {
      const { data, error } = await client
        .from("announcements")
        .select("id, title, body, created_by, created_at, updated_at, profiles!announcements_created_by_fkey(name)")
        .order("created_at", { ascending: false });
      if (error) return [];
      return data.map((a: any) => ({
        ...
        title: a.title,
        body: a.body,
        createdBy: a.profiles?.name ?? "Admin",
...
export const sendAnnouncement = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    return safeQuery(
      ...
      { ok: false, error: "Failed to send announcement" },
    );
  });
...
export const deleteAnnouncement = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    return safeQuery(
      async () => {
        const { error } = await client.from("announcements").delete().eq("id", data.id);
        if (error) return { ok: false, error: error.message };
        return { ok: true, message: "Announcement deleted." };
      },
      { ok: false, error: "Failed to delete announcement" },
    );
  });

export const exportUsersCsv = createServerFn({ method: "GET" }).handler(async () => {
  const client = admin();
  if (!client) return "";
  return safeQuery(
    async () => {
      const { data, error } = await client
        .from("profiles")
        .select("email, name, goal, fitness_level, role, daily_calorie_target, created_at")
        .order("created_at", { ascending: false });
      if (error || !data) return "";
      const header = "Email,Name,Goal,Fitness Level,Role,Daily Calorie Target,Created At";
...
export const exportWorkoutsCsv = createServerFn({ method: "GET" }).handler(async () => {
  const client = admin();
  if (!client) return "";
  return safeQuery(
    async () => {
      const { data, error } = await client
        .from("workout_sessions")
        .select("id, user_id, date, status, duration_actual, rating, difficulty_rating, created_at")
        .order("date", { ascending: false })
        .limit(10000);
      if (error || !data) return "";
      const header = "ID,User ID,Date,Status,Duration (min),Rating,Difficulty,Created At";
...
export const exportNutritionCsv = createServerFn({ method: "GET" }).handler(async () => {
  const client = admin();
  if (!client) return "";
  return safeQuery(
    async () => {
      const { data, error } = await client
        .from("food_logs")
        .select("id, user_id, date, meal_type, name, calories, protein_g, carbs_g, fats_g, created_at")
        .order("date", { ascending: false })
        .limit(10000);
      if (error || !data) return "";
      const header = "ID,User ID,Date,Meal Type,Food,Calories,Protein (g),Carbs (g),Fats (g),Created At";
